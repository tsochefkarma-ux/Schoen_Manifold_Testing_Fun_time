...
print("="*79, flush=True)
print("CELL 37B2: DEGREE-THREE RESPONSE RANK-GROWTH AUDIT", flush=True)
print("="*79, flush=True)
...
OUT_SOBJ = os.path.join(RESULTS_DIR, "degree3_response_rank_growth_cell37b2.sobj")
OUT_TXT = os.path.join(RESULTS_DIR, "degree3_response_rank_growth_cell37b2.txt")
TEMPLATE_OUT = os.path.join(RESULTS_DIR, "degree3_transport_selectivity_input_cell37b2.sobj")
PARTIAL_OUT = os.path.join(RESULTS_DIR, "degree3_response_rank_growth_cell37b2_partial.sobj")
...
# Rank-growth audit.  A genuine fifteen-parameter reconstruction from this
# observable requires the combined rank eventually to reach fifteen.
def columns_for_window(sectors, upper):
    cols = []
    exps = list(range(global_min, ZZ(upper)+1))
    for label in labels15:
        col = []
        for sector in sectors:
            col.extend(responses[sector][label]["defect"][ZZ(e)] for e in exps)
        cols.append(col)
    return cols
...
rank_profiles = {}
profile_start = max(ZZ(global_min), ZZ(-4))
for tol_index,tol_here in enumerate(rank_tolerances):
    key = "1e-{}".format([30,40,50][tol_index])
    rank_profiles[key] = {"combined":[], sector_names[0]:[], sector_names[1]:[]}
    for upper in range(profile_start, EXP_MAX+1):
        rc,_ = numerical_rank_columns(columns_for_window(sector_names, upper), tol_here)
        r0,_ = numerical_rank_columns(columns_for_window([sector_names[0]], upper), tol_here)
        r1,_ = numerical_rank_columns(columns_for_window([sector_names[1]], upper), tol_here)
        rank_profiles[key]["combined"].append((ZZ(upper),ZZ(rc)))
        rank_profiles[key][sector_names[0]].append((ZZ(upper),ZZ(r0)))
        rank_profiles[key][sector_names[1]].append((ZZ(upper),ZZ(r1)))

sector_ranks = {}
for sector in sector_names:
    sector_ranks[sector],_ = numerical_rank_columns(columns_for_window([sector], EXP_MAX), rank_tol)

edge_nonzero_count = ZZ(0)
edge_max_abs = RR(0)
for sector in sector_names:
    for label in labels15:
        value = abs(CC(responses[sector][label]["defect"][EXP_MAX]))
        if value > zero_tol:
            edge_nonzero_count += 1
        edge_max_abs = max(edge_max_abs, RR(value))
support_hits_upper_edge = bool(edge_nonzero_count > 0)

print("\nPART IV. RANK-GROWTH AND EDGE AUDIT", flush=True)
print("-"*79, flush=True)
print("  sector rank {} : {} / 15".format(sector_names[0],sector_ranks[sector_names[0]]), flush=True)
print("  sector rank {} : {} / 15".format(sector_names[1],sector_ranks[sector_names[1]]), flush=True)
print("  combined rank at U^{} : {} / 15".format(EXP_MAX,rank_G), flush=True)
print("  nonzero columns at upper edge: {} / 30".format(edge_nonzero_count), flush=True)
print("  largest |coefficient at upper edge|: {}".format(edge_max_abs), flush=True)
print("  support reaches census boundary? {}".format(support_hits_upper_edge), flush=True)
for key in ["1e-30","1e-40","1e-50"]:
    profile = rank_profiles[key]["combined"]
    jumps = []
    last = None
    for upper,rank_here in profile:
        if rank_here != last:
            jumps.append((upper,rank_here))
            last = rank_here
    print("  combined rank jumps at tolerance {}: {}".format(key,jumps), flush=True)

print("\nPART V. MAXIMAL SUPPORT-MATCHED ABSORPTION DIAGNOSTIC", flush=True)
print("-"*79, flush=True)
...
certificate = {
    "schema_version": ZZ(1),
    "scope": "high-precision degree-three response rank-growth and Laurent-boundary audit",
    "precision_bits": PREC_BITS,
    "laurent_precision": LAURENT_PREC,
    ...
    "sector_ranks_numerical": sector_ranks,
    "rank_profiles": rank_profiles,
    "support_hits_upper_edge": support_hits_upper_edge,
    "edge_nonzero_count": edge_nonzero_count,
    "edge_max_abs": edge_max_abs,
    "requested_exp_max": EXP_MAX_REQUESTED,
    ...
    "next_step": "Only after rank/support stability: derive admissible p^3 connection or broaden the observable",
}
...
template = {
    ...
    "warning": "If rank(G)<15 stably, this observable cannot uniquely reconstruct fifteen coefficients",
}
...
summary_lines = [
    ...
    "audited census window: [{} , {}]".format(EXP_MIN,EXP_MAX),
    "observed nonzero support in window: [{} , {}]".format(global_min,global_max),
    "support reaches upper boundary: {}".format(support_hits_upper_edge),
    "upper-edge nonzero columns: {} / 30".format(edge_nonzero_count),
    "sector rank {}: {} / 15".format(sector_names[0],sector_ranks[sector_names[0]]),
    "sector rank {}: {} / 15".format(sector_names[1],sector_ranks[sector_names[1]]),
    "combined ambiguity-response rank: {} / 15".format(rank_G),
    ...
    "interpretation: rank(G) is an absolute upper bound on selectivity from this observable",
    "next: if rank(G) stays below 15, stack additional CM points/slices/operators before physical module derivation",
]
